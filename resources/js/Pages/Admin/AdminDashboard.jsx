    ...
    useEffect(() => {
        const fetchFilters = async () => {
            try {
                ...
                setPeripheralTypes(Array.isArray(typesRes.data) ? typesRes.data : []);
                 const equipmentRes = await axios.get("/admin/equipment-types");
            setEquipmentTypes(Array.isArray(equipmentRes.data) ? equipmentRes.data : []);
            } catch (err) {
 ...
 const fetchAllEquipmentData = async () => {
    ...
    const cardData = [
        ...
        },
        {
            ...
            value: animatedValue(
                selectedRoom || selectedType
                    ? Object.values(conditionData.system_units).reduce((a, b) => a + b, 0)
        ...
        },
        {
            ...
            value: animatedValue(
                selectedRoom || selectedType
                    ? Object.values(conditionData.peripherals).reduce((a, b) => a + b, 0)
        ...
        },
        {
            ...
            value: animatedValue(
                selectedRoom || selectedType
                    ? Object.values(conditionData.equipments).reduce((a, b) => a + b, 0)
            ...
            <div className="grid grid-cols-1 lg:grid-cols-6 gap-6 mb-6">
              ...
              <div className="lg:col-span-3">
                ...
                {/** Just paste the full <Card> for Equipment Condition here **/}
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem onClick={() => setSelectedRoom("")}>
                      {selectedRoom === "" && <Check className="w-4 h-4 mr-1" />}
                      All Rooms
                    </DropdownMenuItem>
                    {rooms.map((room) => (
                      <DropdownMenuItem key={room.id} onClick={() => setSelectedRoom(room.id)}>
                        {selectedRoom === room.id && <Check className="w-4 h-4 mr-1" />}
                        {room.room_number}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-72">
                        ...
                        <div>
                          <div className="flex justify-center items-center gap-2 mb-1">
                            <h3 className="font-semibold">Peripherals</h3>
                            <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      size="lg"
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem onClick={() => setSelectedType("")}>
                      {selectedType === "" && <Check className="w-4 h-4 mr-1" />}
                      All Types
                    </DropdownMenuItem>
                    {peripheralTypes.map((type) => (
                      <DropdownMenuItem key={type} onClick={() => setSelectedType(type)}>
                        {selectedType === type && <Check className="w-4 h-4 mr-1" />}
                        {type}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                ...
                <div>
                  <div className="flex justify-center items-center gap-2 mb-1">
                    <h3 className="font-semibold">Equipments</h3>
                  <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      size="lg"
                    ...
                    >
                      <SlidersHorizontal className="w-5 h-5" />
                      {selectedEquipmentType || "All Types"}
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent>
                    <DropdownMenuItem onClick={() => setSelectedEquipmentType("")}>
                      {selectedEquipmentType === "" && <Check className="w-4 h-4 mr-1" />}
                      All Types
                    </DropdownMenuItem>
                    {equipmentTypes.map((type) => (
                      <DropdownMenuItem key={type} onClick={() => setSelectedEquipmentType(type)}>
                        {selectedEquipmentType === type && <Check className="w-4 h-4 mr-1" />}
                        {type}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
              ...
              <div className="lg:col-span-1">
                ...
                {/** Just paste the full <Card> for Total Rooms Assets here **/}
                  ...
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      <SlidersHorizontal className="w-5 h-5" />
                      {selectedRoomRight || "All Rooms"}
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent>
                    <DropdownMenuItem onClick={() => setSelectedRoomRight("")}>
                      {selectedRoomRight === "" && <Check className="w-4 h-4 mr-1" />}
                      All Rooms
                    </DropdownMenuItem>
                    {rooms.map((room) => (
                      <DropdownMenuItem
                        key={room.id}
                        onClick={() => setSelectedRoomRight(room.id)}
                      >
                        {selectedRoomRight === room.id && <Check className="w-4 h-4 mr-1" />}
                        {room.room_number}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                      ...
                      <ResponsiveContainer width="100%" height={300}>
                        <BarChart
                          data={[
                            {
                              name: "Total",
                              Computers: allEquipmentData.system_units
                                ? Object.values(allEquipmentData.system_units).reduce((a, b) => a + b, 0)
                                : 0,
                              Peripherals: allEquipmentData.peripherals_type
                                ? Object.values(allEquipmentData.peripherals_type).reduce((a, b) => a + b, 0)
                                : 0,
                              Equipments: allEquipmentData.equipments
                                ? Object.values(allEquipmentData.equipments).reduce((a, b) => a + b, 0)
                                : 0,
                            },
                          ]}
                        ...
                        >
                          ...
                          <XAxis dataKey="name" />
                          <YAxis />
                          <Tooltip /> {/* Hover to see values */}
                          <Legend />
                          <Bar dataKey="Computers" fill="#59AC77" />
                          <Bar dataKey="Peripherals" fill="#FFD700" />
                          <Bar dataKey="Equipments" fill="#FF6B6B" />
                        </BarChart>
                      </ResponsiveContainer>
                    </CardContent>
                  </Card>
