...
export default function FacultyDashboard({ room, stats, user, activeSection }) {
  ...
  const peripheralConditionData = selectedPeripheralType
    ? prepareChartData(stats.peripherals.by_type_condition?.[selectedPeripheralType])
    : prepareChartData(stats.peripherals.by_condition);

  const equipmentConditionData = selectedEquipmentName
    ? prepareChartData(stats.equipments.by_name_condition?.[selectedEquipmentName])
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        <div className="p-4 md:p-6 w-full space-y-6">
          ...
          <h1 className="text-2xl md:text-3xl font-bold">{room.room_name} Dashboard</h1>

          {/* Totals Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {[
              { title: "Total Computers", value: stats.computers.total },
              { title: "Total Peripherals", value: stats.peripherals.total },
              { title: "Total Equipments", value: stats.equipments.total },
            ].map((item, idx) => (
              <Card key={idx} className="w-full flex flex-col">
                ...
                <CardContent className="flex-1 flex items-center justify-center">
                  <p className="text-xl sm:text-2xl md:text-3xl font-bold">{item.value || 0}</p>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <PieChartCard title="Computers by Condition" data={prepareChartData(stats.computers.by_condition)} colors={colors} />
            <PieChartCard title={selectedPeripheralType ? `Condition for ${selectedPeripheralType}` : "Peripherals by Condition"} data={peripheralConditionData} colors={colors} />
            <PieChartCard title={selectedEquipmentName ? `Condition for ${selectedEquipmentName}` : "Equipments by Condition"} data={equipmentConditionData} colors={colors} />
          </div>
        </div>
      </div>
    </div>
      ...
      <CardContent className="flex-1">
        {data.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="value" fill={barFill} onClick={(d) => onClick(d.name)} />
            </BarChart>
          </ResponsiveContainer>
        ) : (
        ...
        {selected && (
          <p className="mt-2 text-sm text-gray-700">
            ...
            <button className="ml-2 text-blue-600 underline" onClick={reset}>
              Reset
            </button>
          </p>
        )}
